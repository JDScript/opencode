...
Nine commits on top of upstream `v2`, one per concern; `git log --oneline upstream/v2..` is the patch set.

| Commit                                                              | Kind              | What                                                                                                                                     |
| ------------------------------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `feat: embed opencode-web as the UI`                                | seam + fork-only  | `OPENCODE_WEB_UI_DIST` in `packages/cli/script/app-assets.ts`; `.gitmodules` + `web/` → JDScript/opencode-web branch `beta`              |
| `ci: release, update and sync fork builds from this repository`     | seams + fork-only | `release-fork.yml`, `sync-fork.yml`, `install-v2`, `mise.toml`, `packages/cli/src/fork.ts`; two seams in `updater.ts`                    |
...
| `feat(cli): no password on loopback by default, and `opencode web`` | seams             | `server-process.ts` password rules; `server/process.ts` pre-router gate; `commands.ts` alias                                             |
...
| `packages/ai/src/protocols/bedrock-converse.ts`                                                                    | Apply Fable 5.1 thinking binding defaults and beta after body overlays, before signing                |
...
`packages/cli/src/fork.ts`, `packages/core/src/plugin/legacy-v1.ts`, `packages/core/src/plugin/superseded.ts`,
`packages/plugin-openviking/`, `.gitmodules` and the `web/` submodule.
...
- **OpenViking memory is a built-in plugin, `packages/plugin-openviking`.** The v1 npm plugin
  ...
  inert without OpenViking credentials. A configured `@openviking/opencode-plugin` is dropped by the
  supervisor before loading (`plugin/superseded.ts`, one log line), so an upgraded user's config keeps
  working untouched, the v1 code never runs alongside, and the stale manual `mcp.openviking` proxy entry is
  simply overridden by the remote registration; user config is never edited. (The plugin also stands down
  on its own if it sees the v1 package active, for hosts without the supervisor seam.)
  Not carried over: the `viking://` URI guard (v1 threw from `tool.execute.before`; promise hooks
